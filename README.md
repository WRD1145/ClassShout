...
| **定时通知** | 设好时间到点自动发；应用没运行时不会补发，错过太久的只标出来 |
...
待发列表里能看到时间、内容摘要、发给几个班，随时可以取消。

**两条必须知道的边界**（界面上也写了）：

- **只有在应用运行时才会到点发送**。把应用彻底关掉之后不会有人替你发 ——
  这是本地定时的固有边界，要做"关掉也能发"得让服务器来担这件事。
- **错过超过三分钟的不会被补发**。"下课前五分钟提醒交作业"在过期十分钟之后已经没意义，
  自动补发只会在下一节课上突然喊一句不着边际的话。过期的只标出来给人看。

发失败的任务会留在待发列表里（而不是消失），老师能看见并手动重试。

...
| **检查更新（可自定义镜像源）** | 两端设置页里点一下就查有没有新版本；GitHub 慢或不通时换镜像（预置几个，也能自己填），下载地址跟着一起换 |
| **开发者模式** | 在两端设置页的**版本号上连点 10 次**解锁：字体回退诊断、打开配置目录、复制诊断信息 |
...
设置页最底下「关于」里，**在版本号上连点 10 次**（手势借自 ClassIsland，
间隔超过 1.5 秒就重新计数）。解锁后多出：字体回退诊断、打开配置目录、复制诊断信息。

**「版本与更新」卡片里那一行版本号也认同一个手势。** 界面上有两处显示版本号，
而人会去点自己最先看到的那一行 —— 只让其中一处能点开，用户就会以为
"开发者模式没了"（这条是踩过的：加了更新卡片之后，两位用户的第一反应都是
去点卡片里那行）。两处共用同一套规则（`DeveloperTapGesture`），
从任一入口解锁，另一处的开发者内容会立即跟着展开（靠 `DeveloperMode.Changed`）。
...
中继服务器也按同一套规则往 `logs/` 写一份。它记的是**事件**而不是每一次请求：
启动、老师登录、班级授权、谁在哪节课喊了什么、定时发出去没有 ——
框架自己的 Information（含每一条长轮询请求）只留警告以上，否则一晚上就能把磁盘写满。
口令一类的值在写文件前会被抹成「（已隐去）」：界面显示口令是设计如此，
落到磁盘就是另一份暴露面（日志会被打包发给别人看排障、会被备份拷走）。
...
**老师也能登这个网页喊话。** 同一个登录框：管理员进去是控制台，老师进去是
「喊话 + 我的班级」那一页 —— 老师不一定装着 App、也不一定带着手机，
站在教室那台电脑前打开网页就能喊一句。权限与 App 完全一致：只列得出、
也只喊得了**管理员授权给自己**的班级（没授权就往里发会被明确拒绝，
而不是悄悄发出去），来源仍由服务器算（科目按各个班取）。
内置管理员用同一套接口时看到的是全部班级。
